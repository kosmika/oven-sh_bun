...
// against regressions.
// Skipped on Windows: the always-on per-worker stdio path (control
// MessageChannels + setupWorkerStdio + console rebind) adds per-spawn overhead
// that this 15x300-snapshot stress exceeds on Windows CI. The race it guards is
// platform-agnostic and covered on macOS/Linux; re-enable once the per-worker
// stdio overhead is reduced (multiplex over the main control channel).
