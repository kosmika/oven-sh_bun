...
pub fn init_with_runtime(
    ...
    let pm = get();
    // `run_once!` installs whatever `log` the first caller passed. Subsequent
    // callers would otherwise leave the singleton pointing at a stale/freed
    // log — e.g. `Bun.build()`'s per-completion log is freed when the task is
    // dropped. Refresh on every call so error paths in `PackageManagerEnqueue`
    // / `runTasks` always write through the live log.
    // SAFETY: `pm` is the process-lifetime singleton; `log_ptr` aliases the
    // caller's `log` which outlives this call.
